...
def collate_pretrain(graphs):
    ...
    # 由于我们强制batch_size=1，这里应该只有一个图
    assert len(graphs) == 1, f"预训练时应该只有一个图，但得到了{len(graphs)}个图"
    return graphs[0]
...
def select_topk_star_normft(star_khop_graph_big, node_ids, central_node_id):
    ...
    if isinstance(central_node_id, torch.Tensor):
        central_node_id = central_node_id.long()
    else:
        central_node_id = torch.tensor(central_node_id, dtype=torch.long)
...
def get_star_topk_nbs(star_khop_graph_big, central_node_id, khop, select_topk):
    ...
    if isinstance(central_node_id, torch.Tensor):
        nbs.append(central_node_id.long()) # make sure self is added
    else:
        nbs.append(torch.tensor(central_node_id, dtype=torch.long)) # make sure self is added
...
class Dataset:
    def __init__(self, name='tfinance', prefix='../datasets/', labels_have="ng", sp_type='star+norm', debugnum = -1):
        ...
        # 使用GADAM和ARC相同的数据集加载方法
        dataset_path = os.path.join(prefix, actual_name)
        ...
        try:
            ...
            if 'label' in graph.ndata:
                ...
                # 统一标签字段名
                graph.ndata['node_label'] = labels
            elif 'node_label' in graph.ndata:
                labels = graph.ndata['node_label']
                num_anomalies = labels.sum().item()
                anomaly_ratio = num_anomalies / graph.num_nodes()
                print(f"  异常节点数: {num_anomalies}")
                print(f"  异常比例: {anomaly_ratio:.4f}")
                ...
                raise ValueError(f"数据集中没有找到标签字段。可用字段: {list(graph.ndata.keys())}")
                        ...
                        # 检查并生成边标签（两端都为异常才标记为异常）
            if 'edge_label' not in graph.edata:
                if 'node_label' in graph.ndata:
                    src, dst = graph.edges()
                    # 确保索引是long类型
                    src = src.long()
                    dst = dst.long()
                    # 只有当边的两个端点都是异常节点时，边才标记为异常
                    edge_labels = (graph.ndata['node_label'][src] & graph.ndata['node_label'][dst]).long()
                    graph.edata['edge_label'] = edge_labels
                else:
                    graph.edata['edge_label'] = torch.zeros(graph.num_edges(), dtype=torch.long)
            ...
            # 设置图标签（用于prepare_dataset方法）
            self.graph_label = [0]  # 默认标签为0
    ...
    def prepare_dataset(self, total_trials=1):
        ...
        # 保存原始图的副本用于预训练
        self.original_graph = self.graph_list[0].clone()
        ...
        if len(self.graph_list) > 1: # multi-graph
            ...
            for i in tqdm(range(samples)):
                ...
                # 确保边ID是long类型，避免索引类型错误
                self.train_mask_edge.append( torch.tensor(train_graph_tmp.edata[dgl.EID], dtype=torch.long) ) # original edge ids
                self.val_mask_edge.append( torch.tensor(val_graph_tmp.edata[dgl.EID], dtype=torch.long) )
                self.test_mask_edge.append( torch.tensor(test_graph_tmp.edata[dgl.EID], dtype=torch.long) )
    ...
    def get_pretrain_dataloaders(self, batch_size):
        # pre-train loader, 预训练时使用保存的原始图，避免GraphMAE掩码操作的问题
        # 因为prepare_dataset会创建三个相同的图，但GraphMAE无法处理批处理图
        if hasattr(self, 'original_graph'):
            # prepare_dataset已经被调用，使用保存的原始图
            pretrain_graph = self.original_graph
        else:
            # prepare_dataset还没有被调用，使用原始图
            pretrain_graph = self.graph_list[0]
            
        # 确保使用单个图，避免批处理导致的掩码索引错误
        pretrain_dataloader = GraphDataLoader(
            [pretrain_graph], 
            batch_size=1,  # 强制batch_size=1，避免批处理
