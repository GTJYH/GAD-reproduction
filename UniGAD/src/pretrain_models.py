...
class GraphMAE(nn.Module):
    ...
    def encoding_mask_noise(self, g, x, mask_ratio=0.3):
        ...
        print(f"DEBUG: g.num_nodes() = {num_nodes}, x.shape = {x.shape}")
        
        # 确保特征张量的节点数与图的节点数匹配
        if x.shape[0] != num_nodes:
            print(f"ERROR: 特征张量节点数 {x.shape[0]} 与图节点数 {num_nodes} 不匹配")
            # 如果特征张量节点数大于图节点数，截取前num_nodes个
            if x.shape[0] > num_nodes:
                x = x[:num_nodes]
                print(f"截取特征张量到 {x.shape}")
            else:
                raise ValueError(f"特征张量节点数 {x.shape[0]} 小于图节点数 {num_nodes}")
        
        # 检查是否是批处理图
        if hasattr(g, 'batch_size') and g.batch_size > 1:
            print(f"DEBUG: 检测到批处理图，batch_size = {g.batch_size}")
            # 如果是批处理图，解包并只使用第一个图
            graphs = dgl.unbatch(g)
            g = graphs[0]
            x = x[:g.num_nodes()]  # 只取第一个图的特征
            num_nodes = g.num_nodes()
            print(f"DEBUG: 解包后 g.num_nodes() = {num_nodes}, x.shape = {x.shape}")
        ...
        if self._replace_ratio > 0:
            ...
            out_x = x.clone()
            # 使用index_fill_避免PyTorch索引bug
            out_x.index_fill_(0, token_nodes, 0.0)
        ...
        # 使用scatter_add_添加mask token
        mask_token_expanded = self.enc_mask_token.expand(token_nodes.size(0), -1)
        out_x.index_add_(0, token_nodes, mask_token_expanded)
        use_g = g.clone()
    ...
    def mask_attr_prediction(self, g, x):
        ...
        if self._decoder_type not in ("mlp", "linear"):
            # * remask, re-mask
            rep.index_fill_(0, mask_nodes, 0.0)  # 使用index_fill_避免PyTorch索引bug
